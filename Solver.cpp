...
void compare(char* input, char* inoutput, int* length, MPI_Datatype* type) {
    ...
    int position3 = 0;
    if (solution1.size() <= solution2.size() && solution1.size() != 0) {
...
void Solver::sendWork() {//posilani v teto zprave mozna nahradit neblokujicim send

    if (!workRequests->empty()) {
        if (space.empty()) {//nic neposlu protoze nic nemam
        ...
        while (workRequests->size() >= (space.size() - 1)) {
            int foo = 3;
            MPI_Isend(&foo, 1, MPI_INT, workRequests->back(), MESSAGE_WORK_NONE, MPI_COMM_WORLD, &request);
            workRequests->pop_back();
        }
        while (!workRequests->empty()) {
            if (myRank > workRequests->back()) {
                cout << "ZCERNAL SEM" << endl;
                processColor = BLACK;
            }
            WorkMessage m;
            //nechci davat prvni stav--je to zadani
            ...
            MPI_Isend(buffer, position, MPI_PACKED, workRequests->back(), MESSAGE_WORK_DATA, MPI_COMM_WORLD, &request);
    ...
    while (flag) {
        switch (status.MPI_TAG) {//tady staci rozsirit o ostatni messages a rozumim vsemu
            ...
            case MESSAGE_WORK_DATA:
            {
                ...
                actualDepth = message.getItems()->back().getDepth(); //nastavim spravnou aktualni hloubku
